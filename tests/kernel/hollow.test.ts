...
describe('hollow underside', () => {
  ...
  it('hangs a locating ring for each magnet from the ceiling and warns when one hits the brim', () => {
    const o = outline(40, 40);
    const slots = magnetSlotSpecs(sizing, [{ x: 0, y: 0 }]);
    const plain = buildBody(o, [], { ...DEFAULT_HOLLOW, watermark: '' });
    const res = buildBody(o, slots, { ...DEFAULT_HOLLOW, watermark: '' });
    expect(res.warnings).toEqual([]);
    expect(isWatertight(res.soup)).toBe(true);
    // ring adds material below the ceiling: volume grows by the annulus x its height
    const ri = slots[0].radius, ro = ri + DEFAULT_HOLLOW.ringWidth;
    const ringVol = Math.PI * (ro * ro - ri * ri) * DEFAULT_HOLLOW.ringHeight;
    const delta = signedVolume(res.soup) - signedVolume(plain.soup);
    expect(delta).toBeGreaterThan(ringVol * 0.9);
    expect(delta).toBeLessThan(ringVol * 1.3 + 2); // + the embedded 0.1 mm overlapping the ceiling
    expect(countZ(res.soup, 2 - DEFAULT_HOLLOW.ringHeight)).toBeGreaterThan(0);
    // a magnet on the brim cannot get a ring
  ...
  it('raises the watermark on the ceiling, mirrored, clear of the rings, and skips it when there is no room', () => {
    ...
    for (let i = bare.soup.triCount * 9; i < res.soup.triCount * 9; i += 3) {
      expect(p[i + 2]).toBeGreaterThanOrEqual(2 - DEFAULT_HOLLOW.watermarkHeight - 1e-6);
      expect(p[i + 2]).toBeLessThanOrEqual(2.1 + 1e-6);
  ...
  it('puts print-ready supports on the brim and on the void ceiling, never on the void wall', () => {
    ...
    // undo tilt/lift: contacts must sit at local z = 0 (brim) or z = 2 (ceiling)
  ...
  it('hollows a base cut from the real 25 mm OPR file (skips when the set is absent)', () => {
    ...
    expect(child.underside!.depth).toBeCloseTo(2, 6);
